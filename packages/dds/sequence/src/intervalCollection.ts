...
export class LocalIntervalCollection<TInterval extends ISerializableInterval> {
            ...
            serializedInterval.properties = MergeTree.addProperties(
                serializedInterval.properties,
                {
                    [reservedIntervalIdKey]:
                        this.createLegacyId(serializedInterval.start, serializedInterval.end),
                },
            );
        }
    }
    ...
    public getIntervalById(id) {
        ...
        this.mapUntil((interval: TInterval) => {
            if (interval.properties?.[reservedIntervalIdKey] === id) {
                result = interval;
                return false;
            }
            return true;
...
export class IntervalCollection<TInterval extends ISerializableInterval> extends EventEmitter {
    ...
    public removeIntervalById(id) {
        if (this.view) {
            return this.view.deleteIntervalById(id);
        }
    }
}
